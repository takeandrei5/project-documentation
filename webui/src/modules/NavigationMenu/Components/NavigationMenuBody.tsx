			...
			<DndProvider backend={MultiBackend} options={getBackendOptions()}>
				<Tree
					...
					dragPreviewRender={(monitorProps) => {
						...
						return <Typography sx={{ fontSize: '0.75rem', pl: '1rem', color: 'blue' }}>{item.text}</Typography>;
					}}
					rootId={0}
					classes={{
						root: 'Root',
					...
					render={(node, { depth, isOpen, onToggle }) => (
						<>
							<Box component={'span'} sx={{ ml: depth * 0.1, p: 0, pt: '0.75rem', display: 'flex' }}>
								{node.droppable && (
									<ListItemIcon sx={{ minWidth: '0.5rem' }} onClick={onToggle}>
